...
const BranchAndFinYearForm = ({ setIsGlobalOpen }) => {
  ...
  useEffect(() => {
    ...
    let selectedFinYears = dropDownFinYear(
      finYears?.filter((val) => val?.active)
    );
  ...
  const onSubmit = () => {
    ...
    if (selectedBranch) {
      ...
      secureLocalStorage.setItem(
        sessionStorage.getItem("sessionId") + "branchCode",
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-gray-100 w-[400px] h-[280px]">
      <form className="w-full text-sm max-w-md px-6 py-8 bg-white shadow-md rounded-md">
        <div className="mb-4">
          <label
        ...
        <div className="mb-4">
          ...
          <select
            ...
            onChange={(e) => {
              setcurrentFinYear(e.target.value);
            }}
          >
            <option value="" hidden>
