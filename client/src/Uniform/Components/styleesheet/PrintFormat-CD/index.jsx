...
import CuttingDelivery from "./LapDip";
import LapDip from "./LapDip";

export default function PrintFormat({ singleData, docId, flattenedData }) {
  console.log("HIt");
  const findAmount = (qty, price, tax, discountType, disAmount) => {
    ...
    if (discountType == "Flat") {
      ...
      dicountAmount = parseFloat(parseFloat(grossAmount) * percentage).toFixed(
        2
      );
    }
    ...
    return (
      parseFloat(grossAmount || 0) +
        ...
        parseFloat(dicountAmount || 0) || 0
    );
  };
  function getTotals(field) {
    const total = singleData?.CuttingDeliveryDetails?.reduce(
      (accumulator, current) => {
